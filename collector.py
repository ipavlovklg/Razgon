...
def ensure_volume_exists(db_conn, volume_info: VolumeInfo, drive_name: str) -> int:
    ...
    if volume_id:
        # cursor.execute(
        #     "UPDATE volumes SET letter = ?, label = ?, filesystem = ?, drive_name = ? WHERE id = ?",
        #     (volume_info.letter, volume_info.label, volume_info.filesystem, volume_id, drive_name)
        # )
        return volume_id
    else:
        cursor.execute(
...
def scan_single_volume_recursive(db_conn: sqlite3.Connection, volume_id: int, current_path: str, root_drive_path: str, progress_counter, current_volume_letter):
    ...
    if not dir_already_indexed:
        try:
            ...
            print(f"\nPermission denied: {current_path}")
            return
        ...
        for file_name in files_in_dir:
            ...
            try:
                ...
                print_progress(progress_counter['processed_files'], progress_counter['processed_size'])
                if should_stop():
                    return
...
def scan_single_volume(db_conn: sqlite3.Connection, volume_info: VolumeInfo, progress_counter, drive_name: str):
    volume_id = ensure_volume_exists(db_conn, volume_info, drive_name)
    root_path = volume_info.letter + "\\"
    scan_single_volume_recursive(db_conn, volume_id, root_path, root_drive_path=root_path, progress_counter=progress_counter, current_volume_letter=volume_info.letter)
...
def print_progress(current_files: int, current_size: int):
...
def scan_and_index_volumes(db_path: str, target_letters: List[str], drive_name: str):
    ...
    for vol_info in target_volumes:
        print(f"Scanning volume {vol_info.letter} ({vol_info.label})")
...
if __name__ == "__main__":
    ...
    for v in available_vols:
        print(f"  {v.letter} - {v.label} ({v.filesystem})")

    input_letters_str = input(f"Enter volume letters to index ({", ".join(v.letter.rstrip(':') for v in available_vols)}): ")
    input_drive_name = input(f"Enter a drive name to identify (or skip if resuming scan): ")
    target_drives = [f"{letter.strip()}:" for letter in input_letters_str.split() if letter.strip()]

    scan_and_index_volumes(DB_PATH, target_drives, input_drive_name)